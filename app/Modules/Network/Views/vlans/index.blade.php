<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Network / VLAN Management') }}</h2>
            <a href="{{ route('network.help') }}" title="Hilfe & Anleitung"
                                ...
                                @forelse ($vlans as $vlan)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center gap-2">
                                                <!-- DHCP Status -->
                                                @if($vlan->dhcp_enabled && $vlan->dhcp_from && $vlan->dhcp_to)
                                                    @php
                                                        $srv = $vlan->dhcpServer;
                                                        $symbolPaths = [
                                                        ...
                                                        $colorMap = ['blue'=>'text-blue-600','red'=>'text-red-600','green'=>'text-green-600','yellow'=>'text-yellow-500','purple'=>'text-purple-600'];
                                                        $iconPath  = $srv ? ($symbolPaths[$srv->symbol] ?? $symbolPaths['server']) : $symbolPaths['server'];
                                                        $iconColor = $srv ? ($colorMap[$srv->color] ?? 'text-blue-600') : 'text-blue-600';
                                                        $tooltip = 'DHCP: ' . $vlan->dhcp_from . ' – ' . $vlan->dhcp_to . ($srv ? ' | ' . $srv->name : '');
                                                    @endphp
                                                    <span class="inline-flex items-center gap-1" title="{{ $tooltip }}">
                                                        <svg class="w-5 h-5 {{ $iconColor }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $iconPath }}"/>
                                                        </svg>
                                                        @if($srv)
                                                            <span class="text-xs font-medium {{ $iconColor }}">{{ $srv->name }}</span>
                                                        @endif
                                                    </span>
                                                @else
