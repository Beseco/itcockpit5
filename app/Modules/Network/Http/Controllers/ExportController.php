...
class ExportController
{
    ...
    /**
     * Generiert den Netzwerk-Export direkt in eine Datei (ohne SSE).
     * Wird vom Backup-Collector aufgerufen.
     */
    public function generateXlsToFile(string $path, callable $log = null): void
    {
        ini_set('memory_limit', '256M');
        ...
        foreach ($vlans as $index => $vlan) {
            ...
            $sheet->setCellValue('A1', 'VLAN ID:');   $sheet->setCellValue('B1', $vlan->vlan_id);
            $sheet->setCellValue('A2', 'Name:');      $sheet->setCellValue('B2', $vlan->vlan_name);
            $sheet->setCellValue('A3', 'Netzwerk:');  $sheet->setCellValue('B3', $vlan->network_address . '/' . $vlan->cidr_suffix);
            $sheet->setCellValue('A4', 'Gateway:');   $sheet->setCellValue('B4', $vlan->gateway ?? '-');
            $sheet->setCellValue('A5', 'DHCP-Bereich:');
            $sheet->setCellValue('B5', $vlan->dhcp_from
                ? ($vlan->dhcp_from . ' - ' . $vlan->dhcp_to) : 'Nicht konfiguriert');
            $sheet->setCellValue('A6', 'Beschreibung:');
            $sheet->setCellValue('B6', $vlan->description ?? '');
            $sheet->getStyle('A1:A6')->getFont()->setBold(true);
            $sheet->getColumnDimension('A')->setWidth(18);
            ...
            $vlan->ipAddresses()->orderByRaw('INET_ATON(ip_address)')->chunk(200, function ($ips) use ($sheet, $lastCol, &$rowIndex) {
                foreach ($ips as $ip) {
                    ...
                    $sheet->getCell([4, $r])->setValue($ip->is_online ? 'Online' : 'Offline');
                    $sheet->getCell([5, $r])->setValue($ip->ping_ms ?? '');
                    $sheet->getCell([6, $r])->setValue($ip->last_online_at ? $ip->last_online_at->format('d.m.Y H:i') : '');
                    $sheet->getCell([7, $r])->setValue($ip->isInDhcpRange() ? 'Ja' : 'Nein');
                    $sheet->getCell([8, $r])->setValue($ip->comment ?? '');
