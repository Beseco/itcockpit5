...
use App\Models\User;
use App\Modules\Network\Http\Requests\StoreVlanRequest;
...
test('validation fails when vlan_id already exists', function () {
...
test('validation fails when gateway is not in subnet', function () {
    ...
    // Don't call validate() - just check if it fails
    expect($validator->fails())->toBeTrue()
