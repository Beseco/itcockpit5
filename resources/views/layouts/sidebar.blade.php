...
<div class="flex-1 flex flex-col min-h-0">
    ...
    <nav class="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        ...
        {{-- Admin-Bereich --}}
        @if($isSuperAdmin || $user->can('base.users.view') || $user->can('base.roles.view') || $user->can('audit.view') || $user->can('base.modules.manage') || $user->can('base.gruppen.view') || $user->can('base.stellen.view'))
        <div class="pt-3 mt-3 border-t border-gray-700">
            ...
            @if($canSee('stellen', 'base.stellen.view'))
            <a href="{{ route('stellen.index') }}"
               class="group flex items-center px-2 py-1.5 text-sm font-medium rounded-md {{ request()->routeIs('stellen.*') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">
                <svg class="mr-3 flex-shrink-0 h-6 w-6 {{ request()->routeIs('stellen.*') ? 'text-white' : 'text-gray-400 group-hover:text-gray-300' }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
