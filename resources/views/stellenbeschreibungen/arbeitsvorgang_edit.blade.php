<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <a href="{{ route('stellenbeschreibungen.edit', $stellenbeschreibung) }}" class="text-gray-400 hover:text-gray-600">
            ...
            <h2 class="text-xl font-semibold text-gray-800">
                {{ $avLabel }} bearbeiten
                <span class="text-gray-400 font-normal text-base">– {{ $stellenbeschreibung->bezeichnung }}</span>
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-6 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white shadow rounded-lg">
            @if($av->exists)
            <form action="{{ route('sb.av.update', [$stellenbeschreibung, $av]) }}" method="POST">
                @csrf @method('PUT')
            @else
            <form action="{{ route('sb.av.store', $stellenbeschreibung) }}" method="POST">
                @csrf
            @endif
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="w-40">
                        ...
                        <x-text-input id="anteil" name="anteil" type="number" min="0" max="100"
                            class="mt-1 block w-full" value="{{ old('anteil', $av->anteil) }}" required />
                        <x-input-error :messages="$errors->get('anteil')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label value="Beschreibung" />
                        <p class="text-xs text-gray-500 mb-2">Unterstützt Markdown: **fett**, *kursiv*, - Listen, # Überschriften</p>
                ...
                <div class="flex items-center justify-between px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-lg">
                    <a href="{{ route('stellenbeschreibungen.edit', $stellenbeschreibung) }}" class="text-sm text-gray-600 hover:text-gray-900">Abbrechen</a>
    ...
    <script>
        new EasyMDE({
            ...
            toolbar: ['bold','italic','|','unordered-list','ordered-list','|','heading-2','heading-3','|','preview','side-by-side','fullscreen'],
        });
    </script>
    @endpush
</x-app-layout>
