    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('announcements.store') }}" x-data="{ type: '{{ old('type') }}' }">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <select id="type" name="type" x-model="type" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                <option value="info" {{ old('type') === 'info' ? 'selected' : '' }}>{{ __('Info') }}</option>
                                <option value="maintenance" {{ old('type') === 'maintenance' ? 'selected' : '' }}>{{ __('Maintenance') }}</option>
                                <option value="critical" {{ old('type') === 'critical' ? 'selected' : '' }}>{{ __('Critical') }}</option>
                            </select>
