<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (session('skipped_accounts') && count(session('skipped_accounts')) > 0)
                <div class="bg-yellow-50 border border-yellow-300 rounded-lg overflow-hidden">
                    <div class="px-4 py-3 bg-yellow-100 border-b border-yellow-300 flex items-center gap-2">
                        ...
                        <span class="text-sm font-semibold text-yellow-800">
                            Nicht importiert – unbekannte Sachkontonummern ({{ array_sum(session('skipped_accounts')) }} Datensätze)
                        </span>
                    </div>
                    <table class="min-w-full text-sm">
                        <thead class="bg-yellow-50">
                            <tr>
                                <th class="px-4 py-2 text-left text-xs font-medium text-yellow-700 uppercase">Sachkontonr.</th>
                                <th class="px-4 py-2 text-right text-xs font-medium text-yellow-700 uppercase">Anzahl übersprungen</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-yellow-200">
                            @foreach (session('skipped_accounts') as $nr => $cnt)
                                <tr class="bg-white">
                                    <td class="px-4 py-2 font-mono text-gray-700">{{ $nr }}</td>
                                    <td class="px-4 py-2 text-right text-gray-700">{{ $cnt }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
